...
// admin.js
;jQuery(function(b){var a=window.chatConfig;b.get(a.templatesPath,function(d){var c=b(d);var e=window.Application;e.service.soundPlayer=new e.SoundPlayer();e.template.message=c.find("#message").html();e.template.operatorItem=c.find("#operator-item").html();e.template.cannedMessageItem=c.find("#canned-message-item").html();e.template.installDialogContent=c.find("#dialog-install-content").html();e.template.confirmDialog=c.find("#dialog-confirm").html();e.template.formErrorDialog=c.find("#dialog-form-error").html();e.template.selectAvatarContent=c.find("#dialog-select-avatar-content").html();e.template.selectCannedMessageContent=c.find("#dialog-select-canned-message-content").html();e.template.historyListItem=c.find("#history-list-item").html();e.template.historyListDisplayMore=c.find("#history-list-display-more").html();e.template.userInfoPopoverContent=c.find("#user-info-popover-content").html();e.template.tabButtonChat=c.find("#tab-button-chat").html();e.template.tabContentChat=c.find("#tab-content-chat").html();e.model.user=new e.UserModel(window.userData);e.model.uiSettings=new e.UISettingsModel();e.model.cannedMessages=new e.CannedMessagesModel();e.model.settings=new e.AdminSettingsModel();e.model.chat=new e.AdminChatModel();e.model.user.listenTo(e.model.chat,"operator:saved",function(f){if(f.id===this.get("id")){this.set(f)}});e.view.window=new e.WindowView({el:"#customer-chat",model:e.model.chat});e.view.dialogs=new e.DialogsView();if(!a.ui.installed){if(e.model.user.hasRole("ADMIN")){e.view.dialogs.confirm("Install",e.template.installDialogContent,{Install:function(){(document.location||window.location).href=a.installPath}})}else{e.view.dialogs.message("Please install first",'The application is not yet installed, please log in as "admin" and install it before using')}}})});
